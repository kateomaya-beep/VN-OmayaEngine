...
export interface RuntimeState {
  ...
  // НЕВИДИМАЯ OOC-ЗАПИСКА. Уходит в запрос ровно перед ходом игрока, в самое
  // «слышное» место, и нигде не показывается в ленте — это не реплика, а слово
  // автора движку: «прямая речь строго в кавычках», «сегодня Леви простужен»,
  // «не заканчивай ход вопросом».
  //
  // Отдельно от авторских заметок намеренно, хотя соблазн объединить велик.
  // Заметка — это список записей, который живёт «на глубине» вместе с остальными
  // директивами хода и соревнуется с ними за внимание. Здесь глубина не
  // настраивается вовсе, и в этом весь смысл: одно место, всегда последнее перед
  // ходом, и потому исполняется строже всего.
  oocNote?: string;
