...
// Мини-мастерская оформления плеера: акцент, цвет/прозрачность пузыря, шрифт (ссылкой),
// размер текста. Пер-проектная (тема хранится в project.playerTheme). Влияет ТОЛЬКО
// на экран игры (CSS-переменные корня плеера), не на конструктор/библиотеку.

const ACCENT_PRESETS = ['#b18cff', '#e0578b', '#5cc9ff', '#4ade80', '#f5a623', '#ff6b6b', '#ffffff'];
const BUBBLE_PRESETS = ['#100d18', '#000000', '#1a1226', '#241a2e', '#0d1a1e', '#201018', '#14161e'];
  ...
  return (
    <Modal open={open} onClose={onClose} title={L('Мастерская оформления', 'Appearance workshop')}>
      <div style={themeVars(theme)}>
        ...
        {/* Пузырь реплики: цвет + прозрачность */}
        <label className="label mt-5">{L('Пузырь реплики', 'Dialogue bubble')}</label>
        <div className="flex items-center gap-2 mb-2 flex-wrap">
          {BUBBLE_PRESETS.map((c) => (
            <Swatch
              key={c}
              color={c}
              active={eq(theme.bubbleColor, c)}
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-400 w-28 shrink-0">
            {L('Прозрачность', 'Opacity')}: {Math.round(theme.bubbleOpacity * 100)}%
          </span>
          <input
            type="range"
            min={0.2}
            ...
            onChange={(e) => set({ bubbleOpacity: Number(e.target.value) })}
