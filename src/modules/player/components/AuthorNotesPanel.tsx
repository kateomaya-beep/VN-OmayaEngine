...
// Логика «подтверждено = только чтение»: после подтверждения запись не редактируется,
// пока не нажать «Редактировать» (эту логику потом распространим на другие блоки).
export function AuthorNotesPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Локальное состояние редактирования: id -> черновик. Отсутствие id = «подтверждено».
  ...
  const create = () => {
    ...
    setNotes([...notes, n]);
    setDrafts((d) => ({ ...d, [n.id]: '' })); // новая запись сразу в режиме правки
  };
