...
- МЕНЯЙ наряд, когда этого требует контекст: смена локации/времени/деятельности (дом →
  бал, улица → бой, день → сон), переодевание по сюжету. Выбирай ТОЛЬКО из доступных
  ЭТОМУ персонажу нарядов; неизвестный тег движок откатит к дефолтному.
