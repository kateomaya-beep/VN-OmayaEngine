...
// Стоп-строки для провайдера: реплика героя с новой строки. Без перевода строки
// впереди нельзя — имя героя встречается в обычной прозе, и генерация обрывалась бы
// посреди фразы «Кейт: …» в чужих устах.
export function rpStopSequences(userName: string): string[] {
  const name = userName.trim();
  return name ? [`\n${name}:`, `\n**${name}**:`] : [];
}
